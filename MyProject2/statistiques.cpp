...
{    QSqlQuery qry;
   ...
   qry.prepare("select * from avion where type = 'Boeing' ");
     if (qry.exec())
     {
         while (qry.next())
         {
           if(qry.value(2)=="Non Fonctionnel") t11++;
           else if(qry.value(2)=="Fonctionnel")  t12++;

       }
     }
     qry.prepare("select * from avion where type = 'Airbus' ");
       if (qry.exec())
       {
           while (qry.next())
           {
             if(qry.value(2)=="Non Fonctionnel") t21++;
             else if(qry.value(2)=="Fonctionnel")  t22++;

         }
       }
       qry.prepare("select * from avion where type = 'Antonov' ");
         if (qry.exec())
         {
             while (qry.next())
             {
               if(qry.value(2)=="Non Fonctionnel") t31++;
               else if(qry.value(2)=="Fonctionnel")  t32++;
    ...
    ui->customPlot->yAxis->setLabel("Nombre d'avion par type");
